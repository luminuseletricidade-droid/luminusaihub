...
import { ContractAddendums } from '@/components/contract/ContractAddendums';
...
const ContractDataEdit: React.FC<ContractDataEditProps> = ({ 
  ...
  const handlePreviewPDF = async (doc: ContractDocument | OriginalDocument) => {
    try {
        ...
        if (storageFiles && storageFiles.length > 0) {
          ...
          if (pdfFile) {
            ...
            const { data, error } = await supabase.storage
              .from('contract-documents')
              .createSignedUrl(foundPath, 3600);
      ...
      console.log('📤 [PREVIEW] Gerando signed URL para:', filePath);
      // Gerar signed URL com validade de 1 hora
      const { data, error } = await supabase.storage
        .from('contract-documents')
      ...
      console.log('🔗 [PREVIEW] Signed URL gerada:', data.signedUrl);
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              <Input
                type="number"
                step="0.01"
                value={editedData.monthly_value || ''}
                onChange={(e) => setEditedData({...editedData, monthly_value: e.target.value})}
                placeholder="0.00"
              />
            ) : (
              <p className="font-medium text-lg text-blue-600">
                {contractData.monthly_value ? formatCurrency(contractData.monthly_value) : 'Não informado'}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              <Input
                ...
                onChange={(e) => setEditedData({...editedData, duration_months: e.target.value})}
                placeholder="12"
              />
            ) : (
              <p className="font-medium">
                {contractData.duration_months ? `${contractData.duration_months} meses` : 'Não informado'}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              ...
              <p className="font-medium">
                {contractData.start_date ? toBRDateString(contractData.start_date) : 'Não informado'}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              ...
              <p className="font-medium">
                {contractData.end_date ? toBRDateString(contractData.end_date) : 'Não informado'}
              </p>
            )}
          </div>
        </CardContent>
