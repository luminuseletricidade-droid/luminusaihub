...
const TasksByTechnicianChart: React.FC<ChartProps> = ({ data, selectedYear = 'ALL', selectedMonth = 'ALL' }) => {
    const technicianCounts = data.reduce((acc, task) => {
        const technician = task.technician || 'Não Atribuído';
        if (!acc[technician]) {
            acc[technician] = { name: technician, Atrasadas: 0, 'Em Dia': 0, Programadas: 0, Pendentes: 0 };
        }
        ...
        relevantMaintenances.forEach(m => {
            if (m.status === 'EM ATRASO') {
                acc[technician].Atrasadas++;
            } else if (m.status === 'EM DIA') {
                acc[technician]['Em Dia']++;
            } else if (m.status === 'PROGRAMADO') {
                acc[technician].Programadas++;
            } else if (m.status === 'PENDENTE') {
                acc[technician].Pendentes++;
            }
        });
    ...
    const chartData = Object.values(technicianCounts);
...
export default TasksByTechnicianChart;
